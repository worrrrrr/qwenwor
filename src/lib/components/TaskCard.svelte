<script lang="ts">
  ...
  let { task, toggleForm = false }: Props = $props();

  function submitToggle(e: Event) {
    (e.currentTarget as HTMLInputElement).closest('form')?.requestSubmit();
  }
</script>
...
<div class="card card-accent-orange p-5">
  <div class="flex justify-between items-start mb-3">
    ...
    {#if toggleForm}
      <form method="POST" action="?/toggle" use:enhance>
        ...
        <input type="hidden" name="enabled" value={String(!task.enabled)} />
        <label class="relative inline-flex items-center cursor-pointer">
          <input type="checkbox" class="sr-only peer" checked={task.enabled} onchange={submitToggle} />
          {@render toggleSwitch()}
        </label>
      </form>
    {:else}
      <label class="relative inline-flex items-center cursor-not-allowed" title="เปิด/ปิดได้จากหน้า Tasks">
        <input type="checkbox" class="sr-only peer" checked={task.enabled} disabled />
        {@render toggleSwitch()}
      </label>
    {/if}
  </div>
